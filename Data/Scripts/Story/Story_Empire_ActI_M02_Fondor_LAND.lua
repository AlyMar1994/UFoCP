...
function State_M02_Fondor_Begin(message)
	if message == OnEnter then
		DebugMessage("State_M02_Fondor_Begin -- HIT!!!")
		...
		DebugMessage("about to create intro cine!!!")
...
function State_Empire_M02_Fondor_HINT02(message)
	if message == OnEnter then
		...
		for i, building in pairs(structure_list) do
			DebugMessage("adding radar blip to building")
...
function Civilian_Spawn_Loop()
	DebugMessage("Civilian_Spawn_Loop hit!!!")
		...
		for i, building in pairs(structure_list) do
			if building.Get_Hull() then
				...
				if current_civilian_count >= max_civilians_allowed then
					DebugMessage("more than 50 civilians...not spawning!!!")
					break
				end

				DebugMessage("spawning some dudes")
...
function Intro_Cinematic()
	...
	DebugMessage("intro cine function hit!!!")
	...
	if not cine_camera then
		DebugMessage("intro cine Can't find cine_camera!!!")
		return
	end
