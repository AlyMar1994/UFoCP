...
function State_Rebel_A3_M07_Begin(message)
	if message == OnEnter then
		...
		Suspend_AI(1)
		DebugMessage("OnEnter State_Rebel_A3_M07_Begin")
		...
		if not TestValid(antilles) then
				DebugMessage("Missing Captain Antilles")
		end
		...
		if not TestValid(attacker_entry_pos) then
			DebugMessage("Missing Attacker Entry Position")
		end
		...
		if not TestValid(empire_entry_pos) then
			DebugMessage("Missing Empire Entry Position")
		end
		...
		if not TestValid(star_destroyer) then
			DebugMessage("Missing Imperial Star Destroyer")
		end
		...
		if not TestValid(tartan_cruiser_1) then
			DebugMessage("Missing Tartan Cruiser 1")
		end
		...
		if not TestValid(tartan_cruiser_2) then
			DebugMessage("Missing Tartan Cruiser 2")
		end
		...
		--if not TestValid(tartan_cruiser_3) then
			DebugMessage("Missing Tartan Cruiser 3")
		--end
		...
		if not TestValid(victory_destroyer_1) then
			DebugMessage("Missing Victory Destroyer 1")
		end
		...
		if not TestValid(victory_destroyer_2) then
			DebugMessage("Missing Victory Destroyer 2")
		end
		...
		if not TestValid(interdictor_1) then
			DebugMessage("Missing Interdictor 1")
		end
		...
		if not TestValid(interdictor_2) then
			DebugMessage("Missing Interdictor 2")
		end
		...
		if not TestValid(interdictor_3) then
			DebugMessage("Missing Interdictor 3")
		end
		...
		if not TestValid(interdictor_4) then
			DebugMessage("Missing Interdictor 4")
		end
		...
		-- for i,name in pairs(interdictor_marker_names) do
			...
			-- else
				--DebugMessage("Sending interdictor at %s to waypoint %d",name,interdictor_destination_index[i])
		...
		interdictor_list[interdictor_4] = interdictor_destination_index[4]
			...
			-- end
		-- end
		DebugMessage("Done finding interdictors")
...
function Prox_Rebel_Capture(prox_obj, trigger_obj)
	...
	DebugMessage("Rebels captured the pirate")
	Despawn_Pirate()
...
function Story_Mode_Service()
	...
	for unit,i in pairs(interdictor_list) do
		if TestValid(unit) then
			...
			DebugMessage("Interdictor destroyed, if they all go, the pirate will get away.")
	...
	if not one_left then
		if not pirate_disabled then
			DebugMessage("YOU LOSE: All of the interdictors are gone and we haven't disabled the pirate, so he's fleeing.")
...
function Prox_Waypoint(prox_obj, trigger_obj)
	...
	if trigger_obj == pirate_ship and not pirate_disabled then
		...
		DebugMessage("Prox_Waypoint hit")

		if prox_obj == pirate_path_01 then
			DebugMessage("pirate to pirate_path_02")
			trigger_obj.Move_To(pirate_path_02)
		end
		if prox_obj == pirate_path_02 then
			DebugMessage("pirate to pirate_path_03")
