...
function Default_Current_Target_Destroyed(tf)
	...
	Attacking = false
	tf.Activate_Ability("SPREAD_OUT", false) -- Turn off some unending abilities that might no longer be appropriate
end
...
function Default_Hardpoint_Target_In_Range(tf, unit, target)
	DebugMessage("%s -- A hardpoint of %s can now fire on target %s.", tostring(Script), tostring(unit),
		tostring(attacker))
end
